...
│   ├── src/main/resources/songs.csv, songs-extra.csv, songs-recent.csv   bundled song library (27,428 songs total)
...
└── frontend/    React + Vite
    └── src/
        ...
        └── components/   BrowsePanel, SpecDemoPanel, TrendingPanel, RecentHitsPanel, SongTable, StatBar
```
...
| POST   | `/api/songs/reload-sample` | discards whatever was uploaded and restores the bundled 27,428-song dataset |
...
| GET    | `/api/recent-hits`       | ~780 real songs that hit the Billboard top 20, 2021–2026 (see below) |
...
- **`songs-recent.csv`** — 668 real 2021–2026 releases, derived from the same Billboard archive as the Trending tab (see below), filtered down to songs whose *earliest* chart appearance falls in that window — so it's genuinely new music, not old catalog songs re-charting. (A handful of decades-old holiday standards — "White Christmas," "Jingle Bells," etc. — technically first charted on the Hot 100 in this window too, since Billboard only started letting recurring holiday classics chart via streaming in 2020; those are filtered out here by checking for the telltale sign of a real catalog evergreen, charting in the same Nov–Jan window across multiple separate years, and are left to the Trending tab below instead, where that framing is disclosed.)
